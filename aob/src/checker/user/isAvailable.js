...
export default checker(
	...
	async function(){
		if(this.pass("work_am") === false && this.pass("work_pm") === false) this.sender("bad_request", "availability.wrong_work_ampm");

		let availability = await this.method(
		...
		if(this.pass("activity").groupId !== availability.groupId) this.sender("bad_request", "availability.wrong_group");
		
		if(availability === null) this.sender("not_found", "availability.notFound");

		if(this.pass("work_am") === true){
			if(availability.am !== true) this.sender("bad_request", "availability.work_am");
			if(
				availability.work !== null && 
				availability.work.amActivityId !== null &&
				availability.work.amActivityId !== this.pass("activity_id")
			) this.sender("bad_request", "availability.work_am");
		}
		// else if(
		...
		if(this.pass("work_pm") === true){
			if(availability.pm !== true) this.sender("bad_request", "availability.work_pm");
			if(
				availability.work !== null && 
				availability.work.pmActivityId !== null &&
				availability.work.pmActivityId !== this.pass("activity_id")
			) this.sender("bad_request", "availability.work_pm");
		}
		// else if(
